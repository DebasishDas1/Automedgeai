...
from workflows.roofing.prompts import ROOFING_EXPERT_SYSTEM, EXTRACT_FIELDS_SYSTEM
...
async def node_extract_fields(state: dict) -> dict:
    ...
    logger.info("node_complete", node="roof_extract", duration_ms=int((time.perf_counter()-start)*1000))
    return state
...
async def node_chat_reply(state: dict) -> dict:
    ...
    try:
        ...
        messages = [SystemMessage(content=ROOFING_EXPERT_SYSTEM.format(slot_1=slots[0], slot_2=slots[1], slot_3=slots[2]))]
    ...
    logger.info("node_complete", node="roof_reply", duration_ms=int((time.perf_counter()-start)*1000))
    return state
...
async def node_save_and_email(state: dict) -> dict:
    ...
    try:
        async with get_db_context() as db:
            lead = Lead(
                ...
                issue=state.get("issue") or f"Roofing request at {state.get('address')}",
                vertical="roofing",
    ...
    if state.get("email"):
        html = f"<h3>Roofing Consultation Summary</h3><p>Hi {state.get('name')}, we have your request. Our team will contact you shortly.</p>"
        tasks.append(tool_executor.execute("resend", email_tool.send_email, to=state.get("email"), subject="Roofing Consultation Summary", html=html))
    
    await asyncio.gather(*tasks, return_exceptions=True)
    logger.info("node_complete", node="roof_save", duration_ms=int((time.perf_counter()-start)*1000))
    return state